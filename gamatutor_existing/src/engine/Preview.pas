...
begin
  try
   ...
   begin
     // update 10-12-2015 by lukman ----supaya windows always on top audacity
     ...
     begin
       ...
       begin
         ...
         begin
           ...
           begin
             ShellExecute(Self.Handle,nil,pchar(extractfilepath(paramstr(0))+'script\save audacity.exe'), nil, nil, SW_HIDE);
...
begin
   ...
   begin
     ...
     begin
       ShellExecute(Self.Handle,nil,pchar(extractfilepath(paramstr(0))+'script\pause audacity.exe'), nil, nil, SW_HIDE);
...
begin
  ...
  begin
   ...
   if (audacitycommand <> '') then
   begin
     ShellExecute(Self.Handle,nil,pchar(extractfilepath(paramstr(0))+'script\stop audacity.exe'), nil, nil, SW_HIDE);
...
try
 ...
 begin
  ...
  if audacitycommand = '' then
  begin
    ShellExecute(Self.Handle,nil,pchar(extractfilepath(paramstr(0))+'script\open audacity.exe'), nil, nil, SW_HIDE);
    audacitycommand:='openrecord';
  end else
  if (audacitycommand ='openrecord') or (audacitycommand ='stop') then
  begin
    ShellExecute(Self.Handle,nil,pchar(extractfilepath(paramstr(0))+'script\record audacity.exe'), nil, nil, SW_HIDE);
    //sound.RecordSound();
