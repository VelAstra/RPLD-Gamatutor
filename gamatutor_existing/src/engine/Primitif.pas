...
uses
  ...
  Dialogs, StdCtrls, ExtCtrls, ComCtrls, Buttons, jpeg, xtea,mmsystem;
...
type
 ...
 soundwav = class // class untuk merekam suara update tanggal 18-10-2015
 public
   //procedure RecordSound();
   //procedure StopSound();
   //procedure SaveSound();
...
{**************************Implementation of soundwav Class***********************}
{procedure soundwav.RecordSound();
var
...
begin
  ...
  MRet := mciSendString('OPEN NEW TYPE WAVEAUDIO ALIAS mysound', nil, 0, 0);



  MRet := mciSendString('SET mysound TIME FORMAT MS ' +     // set time
  ...
  MRet := mciSendString('RECORD mysound', nil, 0, 0);
 ...
 procedure soundwav.StopSound();
 begin
  mciSendString('STOP mysound', nil, 0, 0)

 end;

  procedure soundwav.PauseSound();
  begin
   mciSendString('PAUSE mysound', nil, 0, 0)

  end;


 procedure soundwav.SaveSound();
 var

   MRet : Integer;

 begin
   MRet := mciSendString(PChar('SAVE mysound "' + 'C:\testing.wav'+'"' ), nil, 0,
    0);
  ...
  MRet := mciSendString('CLOSE mysound', nil, 0, 0) ;
  ...
  showmessage('file suara telah berhasil disimpan');
