...
begin
   ...
   {*write setting file if double click the ANM file* -- update 01-07-2015}
   if (ParamCount = 1) then
   begin
     SettingFileName:=extractfilepath(paramstr(0))+'settingplayer.txt';
     Assignfile(SettingFile,SettingFileName);
     rewrite(SettingFile);
     writeln(settingfile,extractfilepath(paramstr(1)));
     closefile(settingfile);
     form1.FileListBox1.Directory:=extractfilepath(paramstr(1));
   end;
   {---------------------------------------------------------------------}
